...
$(function () {
    ...
    $win.scroll(function () {
        ...
        if (y_offsetWhenScrollDisabled > 0) {
            ...
            $('.mega-menu').addClass('top-50')
            $flagFix = true;
        } else {
            if (!$body.is('.open-header') && !$flagFixO) {
                $('#header').removeClass('fixed');
                $('.mega-menu').removeClass('top-50')
                $flagFix = false;
...
$('.mega-menu').hover(
    function () {
        $(this).parent().find('a').css({ 'color': "#FB5472" })
    },
    function () {
        $(this).parent().find('a').css({ 'color': "#646A89" })
    }
...
// if ($(window).width() < 760) {
//     let parent = $('.mega-menu').parent().find('a')
//     console.log(parent)
//     for (let i = 0; i < parent.length; i++) {
//         console.log('parent', parent[i])
//         let element = parent[i]
//         element.innerHTML = `
//         <button onclick=toggleMegaMenu(${this})>+</button>
//         `+ element.innerHTML
...
const toggleMegaMenu = (element) => {
    console.log('toogle->', element)
}